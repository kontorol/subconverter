...
-   新增 [配置文件](#配置文件) چین `[advanced]` 部分的说明
-   修改调整文档中的多处描述
-   更换文档中失效的外部链接
...
-   新增 [特别用法](#特别用法) چین [规则转换](#规则转换) 的说明
-   修改 [配置文件](#配置文件) 中的 `clash_proxy_group` 为 `proxy_group` ，并增加修改描述与示例
-   修改 [配置文件](#配置文件) چین `[ruleset]` 部分的 `surge_ruleset` 为 `ruleset ` ，并增加修改示例
-   修改 [外部配置](#外部配置) چین `surge_ruleset` 为 `ruleset ` 
-   新增 [外部配置](#外部配置) چین `add_emoji` 和 `remove_old_emoji` 
-   修改 [外部配置](#外部配置) چین `proxy_group` 和  `ruleset ` 的描述与示例
-   调整 [简易用法](#简易用法) 与 [进阶用法](#进阶用法) 中的部分描述
-   更换文档中失效的外部链接

2020/11/20

-   新增 [支持类型](#支持类型) چین `mixed` & `auto` 参数
-   新增 [进阶链接](#进阶链接) 中多个调用参数的说明
-   新增 [配置文件](#配置文件) چین `[userinfo]` 部分的描述
-   新增 [配置文件](#配置文件) چین `[common]`&`[node_pref]`&`[server]` 中多个参数的描述
-   修改 [进阶链接](#进阶链接) چین `url` 参数的说明

2020/04/29

-   新增 [配置文件](#配置文件) 指定默认外部配置文件
-   新增 [配置文件](#配置文件) چین `[aliases]` 参数的描述
...
-   新增 [配置文件](#配置文件) چین `[template]` 参数的描述
-   新增 [外部配置](#外部配置) چین `[template]` 参数的描述
-   新增 [本地生成](#本地生成) 用于在本地生成具体的配置文件
-   新增 [支持类型](#支持类型) چین `mellow` & `trojan` 参数
-   新增 [进阶链接](#进阶链接) چین `new_name` 参数的描述
-   新增 [配置文件](#配置文件) چین `append_sub_userinfo` `clash_use_new_field_name` 参数的描述
-   调整 [说明目录](#说明目录) 层次
...
首先确认需要用到的参数：
target=surge&ver=4 、 tfo=true 、 udp=true 、 emoji=true 、exclude=(ترافیک|官网)
...
| token |  必要 | passwd                    | 为了安全考虑**必须设置token**（详见 [配置文件](#配置文件) چین `[common] 部分` 对 `api_access_token` 的描述） |
...
7.  **exclude_remarks**
    ...
    -   例如:
        ...
        ```ini
        exclude_remarks=(到期|剩余ترافیک|时间|官网|产品|平台)
        ```
...
3.  **rule**
    ...
    -   特殊用法:
        ...
        ```ini
        rule=!!GROUPID=0!!(ترافیک|时间|应急),⌛time
...
2.  **overwrite_original_rules**
    ...
    > 覆盖原有规则，即 `[common]` چین xxx_rule_base 中的内容，设置为 true 时打开，默认为 false
...
4.  **ruleset**
    ...
    ```ini
    ruleset=🍎 اپل,https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Apple.list
    # 表示引用 https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Apple.list 规则
    # 且将此规则指向 [proxy_group] 所设置 🍎 اپل 策略组
    ...
    ruleset=🎯 مستقیم,rules/NobyDa/Surge/Download.list
    # 表示引用本地 rules/NobyDa/Surge/Download.list 规则
    # 且将此规则指向 [proxy_group] 所设置 🎯 مستقیم 策略组
    ruleset=🎯 مستقیم,[]GEOIP,CN
...
# 格式示例
custom_proxy_group=🍎 اپل`url-test`(美国|US)`http://www.gstatic.com/generate_204`300,5,100
# 表示创建一个叫 🍎 اپل 的 url-test 策略组,并向其中添加名字含'美国','US'的节点，每隔300秒测试一次，测速超时为5s，切换节点的延迟容差为100ms
...
custom_proxy_group=انتخاب نود`select`(^(?!.*(美国|日本)).*)
# 表示创建一个叫 انتخاب نود 的 select 策略组,并向其中**依次**添加名字不包含'美国'或'日本'的节点
```
...
;ruleset=🎯 مستقیم,rules/LocalAreaNetwork.list
...
1.  取值
    ...
    # 获取 配置文件 چین clash.http_port 的值
    ```
...
6.  单判断，且如果参数不存在时使用默认值进行判断(可避免请求中无对应参数时发生的报错)
    ...
    dns:
      ...
      nameserver:
       ...
       - 223.5.5.5
       {% endif %}
    # 如果 URL چین doh 参数为 true 时，判断成立。
...
此处 `path` 需要在 [配置文件](#配置文件) چین `template_path` 所限定的路径内
